<?php
	...
	$sql="SELECT SwingType, SkillID, COUNT( SkillID ) , SUM( Value ), MIN( Value ) , MAX( Value ) , SUM( Critical ) , SUM( Special )
							FROM `CombatantHealing` 
 ...
 if ($filter == "Friendly" || $filter == "Enemy" || $filter == "All") {
	$sql="SELECT  Floor(Time/1000), SUM(Value) FROM `CombatantHealing` WHERE EncounterID='".$EncounterID."'  ".$filterSQL." GROUP BY Floor(Time/1000)";
          $result=$conn->query($sql);
           $damage = array();
           $lastTime = 0;
           $total = 0;
           ...
           while($row = $result->fetch_row()){
            ...
            $GenGraph[] = ["type"=>"area", "name" => "Healing Done", "data" => $damage];
}
else {
 		if (count($Skill) > 0) {
	//Individual spell cast
					$sql="SELECT SwingType, GameID, VictimID, SkillID, Time, Value, Critical, Special
							FROM `CombatantHealing` 
					 ...
					 while($row = $result->fetch_assoc()){
					 	...
					 	if ($row['Special'] == 1 ) continue;
					 	if ($row['SwingType'] == 11) $row['Name'] = $row['Name'] . " (*)";
...
function generateDamageTable($Spell, $spellInfo) {
		...
		<tr class=\"MainHead\">
			...
			<th>&nbsp;</td>
			<th data-sort=\"int\" style=\"width: 9%\">Amount</td>
			<th data-sort=\"int\" style=\"width: 14%\">Hit <small>(Low/High)</small></td>
 
			<th data-sort=\"int\" style=\"width: 8%\">Critical</td>
			<th data-sort=\"int\" class=\"MainLast\" style=\"width: 8%\">Interrupt</td>
		</tr>
		</thead>
		<tbody>
		";
...
if (count($Spell) > 0) {
		...
		else if ($s[0] == 21 && $s[1] == 297) {$type = "status"; $dot = ""; }
		else {$type="skill"; $dot = "";}
