<?php
 	...
 	// Append SQL with combatant type
	$filterSQL = "";
	...
	$ZoneNPC = getZoneNPC($_LANG, $Meta['ZoneID']);
		...
		if ($filter == "Friendly" || $filter == "Enemy" || $filter == "All") {
			...
			if ($filter == "All") $filterSQL = "";
			 ...
			 $sql = "SELECT GameID, Type, Name, Job, OwnerID, TotalDamage FROM `Combatant` WHERE EncounterID = '".$EncounterID."' ".$filterSQL."";
					 $result=$conn->query($sql);
					 $Combatant = [];
					 $CombatantDPS = [];
					 $totals = 0;
			...
			// If there is recorded combatants for this type of filter
			if (count($cSQL) > 0) {
				$sql=" SELECT Time, Value, GameID FROM `CombatantDamage` WHERE EncounterID='".$EncounterID."' AND GameID IN (".$ids.") AND SkillID NOT IN (".$limitBreakID.")";
					$result=$conn->query($sql);
					 $damage = array();
					 $total = array();
					 while($row = $result->fetch_assoc()) {
			...
			// If filter is specified to an ID instead
					...
					while($row = $result->fetch_assoc()){
						if ($row['Type'] == 2) $row['Name'] = $ZoneNPC[$row['Name']][0];
					    if ($row['Type'] == 1 && $row['OwnerID'] != 0) $row['Name'] = getName($_LANG, $row['Name']);
					 //	if ($row['Type'] == 1 && $row['OwnerID'] == 0) $row['Name'] = strtoupper(JobID($row['Job']));
				...
				$sql=" SELECT Time, Value, VictimID FROM `CombatantDamage` WHERE EncounterID='".$EncounterID."' AND GameID IN (".$filter.") AND SkillID NOT IN (".$limitBreakID.")";
					$result=$conn->query($sql);
					 $damage = array();
					 $total = array();
					 $idx = [];
...
function generateCombatantDone($Combatant, $Meta) {
		...
		<thead>
		<tr class=\"MainHead\">
			<th data-sort=\"string\" class=\"MainFirst\" style=\"width: 24%\"><span class=\"icon-user\"></span> Damage Breakdown</td>
			<th>&nbsp;</td>
			<th data-sort=\"int\" style=\"width: 19%\">Active</td>
			<th data-sort=\"int\" style=\"width: 12%\">Amount</td>
			<th data-sort=\"int\"  class=\"MainLast\" style=\"width: 15%\">DPS <small>(EncDPS)</small></td>
...
$cc = $Combatant;
 if ($total == 0) $total = 1;
		...
		<tr class=\"MainHead\">
			...
			<th data-sort=\"int\"  class=\"MainLast\" style=\"width: 9%\">DPS</td>
...
$totalCount = "";
if (count($Combatant) > 0) {
		 usort($Combatant, "PartyDmg");
   $count1 = 0;
   $count2 = 0;
  foreach($Combatant as $c) {

 	if ($DPS[$c['GameID']][0] == 9999) $DPS[$c['GameID']][0] = 0;
 	if ($c['OwnerID'] != 0 && isset($cc[$c['OwnerID']])) $owner = "<br /><small style=\"color:gray;\">(".$cc[$c['OwnerID']]['Name'].")</small>";
 	else $owner = "";
 	if (array_values($Combatant)[0]['TotalDamage'] == 0) $dnum = 1;
	else $dnum = array_values($Combatant)[0]['TotalDamage'];
	if ($c['Job'] != 0) {
		$jobpic = JobID($c['Job']);
		...
		$page = $page . "
			...
			<td><div class=\"lengthbar\" style=\"width: ".(($c['TotalDamage']/$dnum)*80)."% \"></div></div></td>
			<td>".$c['TotalDamage']." (".round(($c['TotalDamage']/$total)*100, 2)."%)</td>
			<td>".round($DPS[$c['GameID']][0], 2)."</td>
			<td>".round($DPS[$c['GameID']][1], 2)."</td>
			<td>".round($DPS[$c['GameID']][2], 2)."</td>
			</tr>";
			$count1 += $c['TotalDamage'];
	...
	$totalCount = "<tr style=\"background-color:rgba(0,0,0,0.".$num.");\">
		...
		".$totalCount."
			<tr style=\"background-color:rgba(0,0,0,0.".$num.");\">
			  <td colspan=\"6\" style=\"border-top: 1px #2c333c solid;\"><small  style=\"color:gray;\" data-sort-ignore>*Lowest DPS after 30 seconds has elasped since start of encounter.</small></td>
			</tr></tfoot></table>";
		return preg_replace("@[\\r|\\n|\\t]+@", "", $page);
}
